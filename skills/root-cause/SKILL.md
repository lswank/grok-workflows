...
repository's absolute path, and pass the user's problem, quoted):

```bash
node <repo>/workflows/root-cause.mjs "<problem description>"
```

With evidence files:

```bash
node <repo>/workflows/root-cause.mjs "<problem description> -- /path/to/build.log /path/to/diff.txt"
```
